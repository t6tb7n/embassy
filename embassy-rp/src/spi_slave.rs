...
impl<'d, T: Instance, M: Mode> SpiSlave<'d, T, M> {
    fn new_inner(
        ...
        let presc = Self::apply_config(&inner, &config);
    ...
    /// are applied.
    fn apply_config(inner: &PeripheralRef<'d, T>, config: &Config) -> (u8, u8) {
        let p = inner.regs();
    ...
    pub fn blocking_read(&mut self, data: &mut [u8]) -> Result<(), Error> {
        const FIFO_DEPTH: usize = 8usize;
        let mut rx_remaining = data.len();
        let mut tx_remaining = data.len();

        let p = self.inner.regs();
        while rx_remaining != 0 || tx_remaining != 0 {
            if tx_remaining != 0 && p.sr().read().tnf() && rx_remaining < tx_remaining + FIFO_DEPTH {
                p.dr().write(|w| w.set_data(0));
                tx_remaining -= 1;
            }
            if rx_remaining != 0 && p.sr().read().rne() {
                data[data.len() - rx_remaining] = p.dr().read().data() as u8;
                rx_remaining -= 1;
            }
        }
        Ok(())
...
macro_rules! impl_instance {
    ($type:ident, $irq:ident, $tx_dreq:expr, $rx_dreq:expr, $reset:ident) => {
...
impl<'d, T: Instance, M: Mode> embedded_hal_02::blocking::spi::Transfer<u8> for SpiSlave<'d, T, M> {
    type Error = Error;
...
impl<'d, T: Instance, M: Mode> embedded_hal_02::blocking::spi::Write<u8> for SpiSlave<'d, T, M> {
    type Error = Error;
...
impl<'d, T: Instance, M: Mode> embedded_hal_1::spi::ErrorType for SpiSlave<'d, T, M> {
    type Error = Error;
}

impl<'d, T: Instance, M: Mode> embedded_hal_1::spi::SpiBus<u8> for SpiSlave<'d, T, M> {
...
impl<'d, T: Instance> embedded_hal_async::spi::SpiBus<u8> for SpiSlave<'d, T, Async> {
...
impl<'d, T: Instance, M: Mode> SetConfig for SpiSlave<'d, T, M> {
    type Config = Config;
    type ConfigError = ();
